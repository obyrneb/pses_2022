...
pses_org <- read.csv(
  file.path(data_dir,"Subset 7 - 2022_2023 PSES open dataset - with labels_Sous-ensemble 7 - Ensemble de données ouvertes du SAFF de 2022_23.csv"),
  encoding = "latin1",
  na.strings = c("9999", "X")
) %>% 
...
for (this_unitcode in sector_list) {
  ...
  sector_qs <- sector_qs %>% 
    filter(BYCOND %>% str_detect(unit_level) | unitcode %in% c(0,this_dept)) %>% 
